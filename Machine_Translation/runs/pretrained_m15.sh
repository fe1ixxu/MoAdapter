...
SAVE_PATH=/brtx/606-nvme1/haoranxu/moa_checkpoints/pretrained_m15_size${SIZE}
MAX_UPDATES=100000
ARCH=transformer
FREQ=1
MAX_TOKENS=8192
export WANDB_NAME=pretrained_m15_size${SIZE}
...
python train.py ${DATA_BIN} --arch ${ARCH}  --task translation_multi_simple_epoch \
 ...
 --decoder-normalize-before --encoder-normalize-before --share-all-embeddings --max-source-positions 1024 --max-target-positions 1024 \
...
for src in ${SRCS//,/ }; do
    ...
    cat ${FSRC} |\
    fairseq-interactive ${DATA_BIN} --path $SAVE_PATH/checkpoint_best.pt \
        --langs ${LANGS} \
